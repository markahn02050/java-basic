...
물론 JAVA, Python, CPP등의 프로그래밍과는 달리
...
-- 일반구문에서 LIMIT에는 변수를 입력해서 쓸 수 없습니다.
...
-- 문법
-- CAST(실행문 as 바꿀자료형) FROM 테이블명;
-- CONVERT(실행문, 바꿀자료형) FROM 테이블명;
...
SELECT '가나다' + 100; -- 숫자로 변환할 수 없는 문자 + 정수 연산 => 정수끼리만 변환
...
SELECT CHAR_LENGTH('aspdfjpasjdfpjaspdjfpasf');
...
-- 문자열을 길이만큼 늘려놓고, 빈 칸에 문자열을 채워줍니다.
...
-- TRIM은 기본적으로는 LTRIM + RTRIM형식으로 양쪽의 모든 공백을 다 삭제해줍니다.
-- 공백이 아닌 특정 문자를 삭제하도록 구문을 지정할 수도 있습니다
SELECT TRIM('        무야호     ');

-- 방향은 BOTH(양쪽다), LEADING(왼쪽), TRAILING(오른쪽)
-- 중 하나를 고르면 됩니다.
SELECT TRIM(BOTH 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋㅋㅋㅋ');
...
SELECT REVERSE('asdfasdfasdfasdf');
...
SELECT SUBSTRING('자바스프링마이에스켈', 6, 4);
...
-- getUser()를 만들어주세요. userTble을 조회해줍니다.
-- 만들고 나서 호출까지 해주세요.
use ict_practice;
DELIMITER $$
...
use employees;
DELIMITER $$
CREATE PROCEDURE checkFiveYears()
BEGIN
	DECLARE hireDATE DATE; # 입사일 얻어오기
    DECLARE todayDATE DATE; # 오늘 날짜 얻어오기
    DECLARE days INT; # 오늘날짜 - 입사일해서 경과일수 얻어오기
    
    SELECT hire_date INTO hireDATE
		FROM employees WHERE emp_no = '10001';
	# hire_date INTO hireDATE는
    # 위 쿼리문의 결과로 나온 10001번의 직원의 hire_date의 값을 hireDATE에 저장해줍니다.
    
    SET todayDATE = CURRENT_DATE(); # 오늘날짜를 구해오는 기본기능
    SET days = DATEDIFF(todaytDate, hireDATE); # todaysDATE - hireDATE를 수행
...
use employees;
CREATE PROCEDURE checkFiveYears2(
	emp_number INTEGER
